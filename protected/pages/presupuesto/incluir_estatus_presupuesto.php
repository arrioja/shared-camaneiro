 ...
 * Descripción: Implementa el estatus del presupuesto (sea cual sea su tipo)
 *              para lo cual existen 24 posibles estatus presupuestarios:
 *              PA: "Pasado". Son los presupuestos que se han cerrado, no es posible
 ...
 *              PR: "Presente". Es el presupuesto actual, le aplican todas las
 *                  operaciones normales que tiendan a modificarlo, y ejecutarlo
 *                  conforme a las leyes. Solo permite un cambio posible de estatus
 *                  a "Pasado" una vez que el año fiscal haya oficialmente concluido.
 *              FU: "Futuro". Son los presupuestos que aún no se pueden ejecutar
 *                  y que solo se encuentran en fase de formulación. Ninguna
 *                  operación que pretenda comprometerlo, causarlo, pagarlo etc
 *                  le afecta. Su cambio posible de estatus es a: "Presente"
 *                  siempre y cuando no exista un presupuesto "Presente" previamente,
 *                  lo que quiere decir que antes de que el presupuesto "Futuro"
...
class incluir_estatus_presupuesto extends TPage
{
    ...
    function incluir_click($sender, $param)
    {
        if ($this->IsValid)
        {
            ...
            $sql = "insert into presupuesto.estatus_presupuesto
                    (cod_organizacion, ano, estatus)
                    values ('$cod_organizacion','$ano','$estatus')";
            $resultado=modificar_data($sql,$sender);

            /* Se incluye el rastro en el archivo de bitácora */
            $descripcion_log = "Insertado estatus ".$estatus." de Presupuesto para el año:".$ano;
