 ...
 * Descripción: En esta página incluyen traslados entre codigos presupuestarios.
 *****************************************************  FIN DE INFO
*/
class incluir_traslado extends TPage
{
        ...
        if(!$this->IsPostBack)
          {
              ...
              $this->lbl_total_disminucion->Text="0.00";
 ...
 */
    ...
    public function nuevo_item($sender,$param)
    {
        ...
        {
            if($item->disminucion->Text == "0.00")
              {$item->disminucion->Text = "***";}
            else
              {$item->disminucion->Text = "Bs. ".number_format($item->disminucion->Text, 2, ',', '.');}
            if($item->aumento->Text == "0.00")
              {$item->aumento->Text = "***";}
            else
              {$item->aumento->Text = "Bs. ".number_format($item->aumento->Text, 2, ',', '.');}
        }
    }
 ...
 */
    public function validar_incluido($sender, $param)
    {
       ...
       $codigo = descomponer_codigo_gasto(rellena_derecha($this->txt_codigo->Text,'24','0'));
       $sql = "select * from presupuesto.temporal_traslados
                    where ((cod_organizacion = '$cod_organizacion') and
...
/* Esta función valida que el monto a comprometer tenga disponibilidad */
    public function validar_monto($sender, $param)
    {
       if ($this->cedente->Checked) 
       {
           ...
           if ($registro[0]['disponible'] >= $monto)
           {
               $param->IsValid = true;
           }
           else
       ...
       // si el codigo presupuestario es receptor, no me importa que no tenga disponibilidad.
       if ($this->receptora->Checked)
       {
           $param->IsValid = true;
       }
    }
 ...
 */
    function actualiza_listado()
    {
        ...
        $sql = "select id, CONCAT(sector,'-',programa,'-',subprograma,'-',proyecto,'-',actividad,'-',partida,'-',
              ...
              monto_disminucion, monto_aumento from presupuesto.temporal_traslados
        ...
        $habilitar = !(empty($datos));

        // se actualiza la sumatoria total
        $sql = "select SUM(monto_disminucion) as total_disminucion, SUM(monto_aumento) as total_aumento
                from presupuesto.temporal_traslados
                where ((cod_organizacion = '$cod_organizacion') and (numero = '$numero'))";
        $datos = cargar_data($sql,$this);
        // para evitar que se muestre null cuando viene vacio // disminuciones
        if (empty($datos[0]['total_disminucion']) == false)
        { $this->lbl_total_disminucion->Text=$datos[0]['total_disminucion'];}
        else
        { $this->lbl_total_disminucion->Text="0.00";}
       // para evitar que se muestre null cuando viene vacio // aumentos
        ...
        // si los montos no coinciden se deshabilita el boton de incluir
        if (($this->lbl_total_aumento->Text == $this->lbl_total_disminucion->Text) && ($habilitar == true))
 ...
 */
    function anadir_click($sender, $param)
    {
        if ($this->IsValid)
        {
            ...
            $cod_organizacion = usuario_actual('cod_organizacion');
            $monto = $this->txt_monto->Text;
            // dependiendo de lo que este chequeado, incluyo en una columna yu otra
            if ($this->cedente->Checked) {$columna="monto_disminucion";}
            if ($this->receptora->Checked) {$columna="monto_aumento";}

            $sql = "insert into presupuesto.temporal_traslados
                        (ano, cod_organizacion, numero, sector, programa, subprograma, proyecto, actividad, partida,
                         generica, especifica, subespecifica, ordinal, cod_fuente_financiamiento, $columna)
...
/* esta funcion elimina un codigo presupuestario del listado */
	public function eliminar($sender,$param)
	{
        ...
        $sql="DELETE FROM presupuesto.temporal_traslados WHERE id='$id'";
    ...
    function incluir_click($sender, $param)
    {
        if ($this->IsValid)
        {
            ...
            $criterios_adicionales=array('cod_organizacion' => $cod_organizacion,
                                          'ano' => $ano);
            $numero=proximo_numero("presupuesto.maestro_traslados","numero",$criterios_adicionales,$this);
            ...
            $sql = "insert into presupuesto.maestro_traslados
            ...
            $sql = "insert into presupuesto.detalle_traslados
                    (ano, cod_organizacion, numero, sector, programa, subprograma, proyecto, actividad, partida,
                     generica, especifica, subespecifica, ordinal, cod_fuente_financiamiento, monto_disminucion, monto_aumento)
	   					select t.ano, t.cod_organizacion, t.numero, t.sector, t.programa, t.subprograma, t.proyecto, t.actividad, t.partida,
                        t.generica, t.especifica, t.subespecifica, t.ordinal, t.cod_fuente_financiamiento, t.monto_disminucion, t.monto_aumento
                        from presupuesto.temporal_traslados as t where (t.numero='$aleatorio')";
            $resultado=modificar_data($sql,$sender);

            // Se cambia el numero aleatorio por el numero que realmente que debe tener.
            $sql = "update presupuesto.detalle_traslados set numero='$numero' where (numero='$aleatorio')";
            $resultado=modificar_data($sql,$sender);
            ...
            $sql = "delete from presupuesto.temporal_traslados where (numero='$aleatorio')";
            $resultado=modificar_data($sql,$sender);

            /* Se incluye el rastro en el archivo de bitácora */
            $descripcion_log = "Se ha incluido el traspaso presupuestario: ".$numero. " / año: ".$ano.
