 ...
 * Descripción: Esta página presenta un listado de los recortes al presupuesto que
 *              la institución ha sufrido en el año seleccionado.
 *****************************************************  FIN DE INFO
*/

class listar_recorte extends TPage
{
...
/* este procedimiento cambia la fecha a formato normal antes de mostrarla en el grid */
	...
	public function actualiza_listado1()
	{
        ...
        $sql="select id, num_documento, fecha, motivo, monto_total from presupuesto.maestro_recortes
	...
	public function actualiza_listado2($sender,$param)
	{
        ...
        $sql="select ano, cod_organizacion, numero, num_documento from presupuesto.maestro_recortes
              where (id = '$id2')";
        $resultado=cargar_data($sql,$this);
        ...
        $this->DataGrid2->Caption="Detalle del Recorte Presupuestario (".$resultado[0]['num_documento']."):";
        $sql="select CONCAT(sector,'-',programa,'-',subprograma,'-',proyecto,'-',actividad,'-',partida,'-',
              ...
              monto_disminucion from presupuesto.detalle_recortes
...
/* Esta función permite imprimir un listado de los recortes presupuestarios en el año seleccionado.
 */
    public function imprimir_listado($sender, $param)
    {
        ...
        { // si la consulta trae resultados, entonces si se imprime
            ...
            $info_adicional= "Recortes Presupuestarios. \n".
                             "Año: ".$ano;
            ...
            $pdf->SetTitle('Recortes Presupuestarios.');
            $pdf->SetSubject('Listado de Recortes Presupuestarios de un año.');
            ...
            $pdf->Cell(0, 6, "Recortes Presupuestarios del año ".$ano, 0, 1, '', 1);
            ...
            $pdf->Output("recortes_presupuestarios_".$ano.".pdf",'D');
        }
    }
 ...
 */
    public function imprimir_item($sender, $param)
    {
        ...
        $sql="select ano, cod_organizacion, monto_total, fecha, motivo, numero, num_documento from presupuesto.maestro_recortes
              where (id = '$id2')";
        $resultado=cargar_data($sql,$this);
        ...
        { // si la consulta trae resultados, entonces si se imprime
            ...
            $info_adicional= "Detalle de Recorte Presupuestario. \n".
            ...
            $pdf->SetAuthor('Proyecto SIMON');
            $pdf->SetTitle('Detalle de Recortes Presupuestarios.');
            $pdf->SetSubject('Detalle de Recortes Presupuestarios.');
            ...
            $pdf->SetFont('helvetica', '', 14);
            $pdf->SetFillColor(255, 255, 255);
            $pdf->Cell(0, 6, "Códigos Presupuestario afectados por el recorte", 0, 1, '', 1);
            $pdf->SetFont('helvetica', '', 12);
            // se realiza el listado de asistentes en el PDF
            $pdf->SetFillColor(210,210,210);
            $pdf->SetTextColor(0);
            ...
            $pdf->Output("detalle_recorte_presupuestario_".$numero.".pdf",'D');
        }
    }



/* Esta función imprime todos los códigos presupuestarios de los recortes
 ...
 */
    public function imprimir_codigos($sender, $param)
    {
        ...
        $sql="select distinct CONCAT(sector,'-',programa,'-',subprograma,'-',proyecto,'-',actividad,'-',partida,'-',
                            generica,'-',especifica,'-',subespecifica,'-',ordinal) as codigo,
              SUM(monto_disminucion) as monto from presupuesto.detalle_recortes
              where ((cod_organizacion = '$cod_organizacion') and (ano = '$ano'))
              group by codigo";
        $resultado_rpt=cargar_data($sql,$this);
        ...
        { // si la consulta trae resultados, entonces si se imprime
            ...
            $info_adicional= "Detalle de Códigos presupuestarios afectados por recortes. \n".
                             "Año: ".$ano;
            $pdf->SetHeaderData("LogoCENE.gif", 15, "Sistema de Información Automatizado - ".usuario_actual('nombre_organizacion'), $info_adicional);
            ...
            $pdf->SetFont('helvetica', '', 14);
            $pdf->SetFillColor(255, 255, 255);
            $pdf->Cell(0, 6, "Códigos presupuestarios afectados por recortes", 0, 1, '', 1);
            $pdf->SetFont('helvetica', '', 12);
            // se realiza el listado de asistentes en el PDF
            $pdf->SetFillColor(210,210,210);
            $pdf->SetTextColor(0);
            ...
            $pdf->Output("cod_afectados_en_recortes_".$ano.".pdf",'D');
        }
    }
