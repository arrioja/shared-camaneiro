 ...
 * Descripción: En esta página incluyen recortes al presupuesto
 *              de gastos.
 *****************************************************  FIN DE INFO
*/
class incluir_recorte extends TPage
{
 ...
 */
    public function validar_incluido($sender, $param)
    {
       ...
       $codigo = descomponer_codigo_gasto(rellena_derecha($this->txt_codigo->Text,'24','0'));
       $sql = "select * from presupuesto.temporal_recortes
                    where ((cod_organizacion = '$cod_organizacion') and
 ...
 */
    function actualiza_listado()
    {
        ...
        $sql = "select id, CONCAT(sector,'-',programa,'-',subprograma,'-',proyecto,'-',actividad,'-',partida,'-',
              ...
              monto_disminucion from presupuesto.temporal_recortes
        ...
        $sql = "select SUM(monto_disminucion) as total_disminucion
                from presupuesto.temporal_recortes
                where ((cod_organizacion = '$cod_organizacion') and (numero = '$numero'))";
        $datos = cargar_data($sql,$this);
       // para evitar que se muestre null cuando viene vacio // aumentos
        if (empty($datos[0]['total_disminucion']) == false)
        { $this->lbl_total_aumento->Text=$datos[0]['total_disminucion'];}
        else
        { $this->lbl_total_aumento->Text="0.00";}
 ...
 */
    function anadir_click($sender, $param)
    {
        if ($this->IsValid)
        {
            ...
            $sql = "insert into presupuesto.temporal_recortes
                        (ano, cod_organizacion, numero, sector, programa, subprograma, proyecto, actividad, partida,
                         generica, especifica, subespecifica, ordinal, cod_fuente_financiamiento, monto_disminucion)
...
/* esta funcion elimina un codigo presupuestario del listado */
	public function eliminar($sender,$param)
	{
        ...
        $sql="DELETE FROM presupuesto.temporal_recortes WHERE id='$id'";
    ...
    function incluir_click($sender, $param)
    {
        if ($this->IsValid)
        {
            ...
            $criterios_adicionales=array('cod_organizacion' => $cod_organizacion,
                                          'ano' => $ano);
            $numero=proximo_numero("presupuesto.maestro_recortes","numero",$criterios_adicionales,$this);
            ...
            $sql = "insert into presupuesto.maestro_recortes
            ...
            $sql = "insert into presupuesto.detalle_recortes
                    (ano, cod_organizacion, numero, sector, programa, subprograma, proyecto, actividad, partida,
                     generica, especifica, subespecifica, ordinal, cod_fuente_financiamiento, monto_disminucion)
	   					select t.ano, t.cod_organizacion, t.numero, t.sector, t.programa, t.subprograma, t.proyecto, t.actividad, t.partida,
                        t.generica, t.especifica, t.subespecifica, t.ordinal, t.cod_fuente_financiamiento, t.monto_disminucion
                        from presupuesto.temporal_recortes as t where (t.numero='$aleatorio')";
            $resultado=modificar_data($sql,$sender);

            // Se cambia el numero aleatorio por el numero que realmente que debe tener.
            $sql = "update presupuesto.detalle_recortes set numero='$numero' where (numero='$aleatorio')";
            $resultado=modificar_data($sql,$sender);
            ...
            $sql = "select CONCAT(sector,'-',programa,'-',subprograma,'-',proyecto,'-',actividad,'-',partida,'-',
                            generica,'-',especifica,'-',subespecifica,'-',ordinal) as codigo, monto_disminucion
              from presupuesto.temporal_recortes
            ...
            $sql = "delete from presupuesto.temporal_recortes where (numero='$aleatorio')";
            $resultado=modificar_data($sql,$sender);

            /* Se incluye el rastro en el archivo de bitácora */
            $descripcion_log = "Se ha incluido el Recorte Presupuestario: ".$numero. " / año: ".$ano.
