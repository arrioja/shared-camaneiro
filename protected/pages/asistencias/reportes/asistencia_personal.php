 ...
 * Descripción: Este es un reporte de asistencias e inasistencias para el funcionario conectado
 *              en un rango de fechas dadas.
 *****************************************************  FIN DE INFO
*/
include("protected/comunes/libchart/classes/libchart.php");
class asistencia_personal extends TPage
{
    ...
    /* esto se usa para mostrar un listado de autocompletar de las cedulas que
     * pertenezcan a la misma organizacion que el usuario
     */
    public function buscar_cedula($sender, TCallbackEventParameter $param)
    {
/*        $busca = $param->getToken();
        $cod_organizacion = usuario_actual('cod_organizacion');
        $sql="select distinct p.cedula
              from organizacion.personas p, organizacion.personas_nivel_dir n
              where ((n.cedula = p.cedula) and (n.cod_organizacion = '$cod_organizacion') and (p.cedula like '$busca%'))";
        $data = cargar_data($sql,$this);
        $this->txt_cedula->setDataSource($data);
        $this->txt_cedula->dataBind();*/
    }
 ...
 * inasistentes con los correspondientes gráficos y observaciones*/
    public function consulta_asistencia($sender, $param)
    {
        if ($this->IsValid)
        {
            ...
            while ($contador < count($this->diasasistencia))
            {
                if (($this->diasasistencia[$contador]['entrada'] == "") && ($this->diasasistencia[$contador]['entrada'] == "") &&
                    ($this->diasasistencia[$contador]['observacion'] == ""))
                    {
                        ...
                        {
                            $result  = esta_justificado($this->justificaciones,$cedula,$this->diasasistencia[$contador]['fecha'],$horario_vigente[0]['hora_entrada'],$sender);
                            $result2 = esta_justificado($this->justificaciones,$cedula,$this->diasasistencia[$contador]['fecha'],$horario_vigente[0]['hora_salida'],$sender);
            ...
            $chart->setTitle("Asistencias / Inasistencias de: ".$cedula);
        ...
        else
        {
            ...
            if (($item->entrada->Text != "Entrada") and (($item->entrada->Text != "")))
            {
                if (strtotime($item->entrada->Text) > (strtotime($horario_vigente[0]['hora_entrada']." + ".
                   ...
                   {
                       if (esta_justificado($this->justificaciones,$cedula,$item->fecha->Text,$item->entrada->Text,$sender) != false)
                         {
                             $item->entrada->ForeColor = "Green";
                             $this->ind_asistentes_tarde_si_just++;
            ...
            if (($item->salida->Text != "Salida") and ($item->salida->Text != ""))
            {
                ...
                {
                   if (esta_justificado($this->justificaciones,$cedula,$item->fecha->Text,$item->salida->Text,$sender) != false)
                       {
    ...
    public function imprimir_asistencia($sender, $param)
    {
     ...
     //   $pdf=new TCPDF('p', 'mm', 'letter', true, 'iso-8859-1', false);
            ...
            else
            {
                if (strtotime($row['entrada'])>= (strtotime($horario_vigente_rpt[0]['hora_entrada']." + ".
                   ...
                   {// si llegó tarde, ahora se comprueba la justificacion
                       if (esta_justificado($this->justificaciones,$cedula,$row['fecha'],$row['entrada'],$sender) != false)
                ...
                { // si salió temprano, se comprueban las justificaciones
                   if (esta_justificado($this->justificaciones,$cedula,$row['fecha'],$row['salida'],$sender) != false)
       ...
       $pdf->Ln();$pdf->Ln();
        ...
        $pdf->Output("asistencia_propia_".$cedula."_del_".$desde."_al_".$hasta.".pdf",'D');
    }
