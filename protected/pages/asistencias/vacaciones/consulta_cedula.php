 ...
 * Descripción: Esta ventana consulta las vacaciones disfrutadas y pendientes de
 *              el funcionario titular de la cédula dada..
 *****************************************************  FIN DE INFO
*/
class consulta_cedula extends TPage
{
    ...
    {      
       ...
       if (!$this->IsPostBack)
       {
           ...
           $cod_org = usuario_actual('cod_organizacion');
            ...
            $this->txt_cedula->DataSource=$resultado;
            $this->txt_cedula->dataBind();
            
          if (isset($this->Request['ced']))
            {
               ...
               if($estado=="1"){//si esta aprobada
                ...
                { // si la consulta trae resultados, entonces si se imprime
                    ...
                    for ($num_solicitud = 0 ; $num_solicitud < 2 ; $num_solicitud++) {
                        $pdf->Ln();
                        $pdf->Cell(0, 0, "Los Robles, $f2[2]/$f2[1]/$f2[0]", 0, 0, 'R', 1);
                        $pdf->Ln();
