<?php
class editar_jubilacion extends TPage
{
    ...
    public function guardar($sender, $param)
    {
        ...
        $documento=$this->oculto3->text;
        $sql2="update expedientes.jubilacion set fecha='$fecha', descripcion='$descripcion', documento='$documento'
        where(id='$id')";
    ...
    function cargar($sender, $param)
    {
      ...
      if($sender->HasFile)
      {
         ...
         $this->oculto3->text='';
         $this->oculto3->text=$this->oculto3->text."$ruta$sender->FileName"."\n";
    ...
    function volver_jubilacion($sender, $param)
    {
        $cedula=$this->Request['cedula'];
        $this->Response->Redirect( $this->Service->constructUrl('expedientes.expediente',array('cedula'=>$cedula)));
    }
