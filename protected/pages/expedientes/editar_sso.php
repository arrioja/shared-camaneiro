<?php
class editar_sso extends TPage
{
    ...
    public function guardar($sender, $param)
    {
        ...
        $documento=$this->oculto4->text;
        $sql2="update expedientes.sso set fecha='$fecha', descripcion='$descripcion', documento='$documento'
        where(id='$id')";
    ...
    function cargar($sender, $param)
    {
      ...
      if($sender->HasFile)
      {
         ...
         $this->oculto4->text='';
         $this->oculto4->text=$this->oculto4->text."$ruta$sender->FileName"."\n";
    ...
    function volver_sso($sender, $param)
    {
        $cedula=$this->Request['cedula'];
        $this->Response->Redirect( $this->Service->constructUrl('expedientes.expediente',array('cedula'=>$cedula)));
    }
