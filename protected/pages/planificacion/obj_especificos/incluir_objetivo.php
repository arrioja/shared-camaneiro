...
class incluir_objetivo extends TPage
{
	...
	public function actualizar_obj_estrategicos($sender, $param)
	{
        ...
        $sql="Select cod_objetivo_operativo, nombre
            From planificacion.objetivos_operativos
 ...
 //           $cod_plan_estrategico = $this->lbl_cod_plan_estrategico->Text;
            $cod_plan_operativo = $this->drop_plan->SelectedValue;
            $cod_objetivo_operativo = $this->drop_objetivo->SelectedValue;

            $titulo = $this->txt_nombre_completo->Text;
            $descripcion = $this->txt_descripcion->Text;

            $codigo = proximo_numero("planificacion.objetivos_especificos","cod_objetivo_especifico",null,$sender);
            $codigo = rellena($codigo,6,"0");

            /* Se Inicia el procedimiento para guardar en la base de datos  */

            $sql="insert into planificacion.objetivos_especificos
                    (cod_organizacion, cod_plan_operativo, cod_objetivo_operativo,
                     cod_objetivo_especifico, nombre,
                     descripcion, porcentaje_completo)
                  value ('$cod_organizacion','$cod_plan_operativo','$cod_objetivo_operativo',
                         '$codigo','$titulo','$descripcion','0')";
            $resultado=modificar_data($sql,$sender);

            /* Se incluye el rastro en el archivo de bitácora */
            $descripcion_log = "Se ha incluido el Objetivo Específico: ".$titulo;
            inserta_rastro(usuario_actual('login'),usuario_actual('cedula'),'I',$descripcion_log,"",$sender);

            $this->Response->redirect($this->Service->constructUrl('planificacion.obj_especificos.incluir_objetivo'));
        }
	}
