...
class nomina_historial_personal extends TPage
{
    ...
    public function pagos($sender,$param)
    {
        ...
        $this->Response->Redirect( $this->Service->constructUrl('nomina.nominas.integrantes_pagos_personal',array('ced'=>$cedula,'cod'=>$cod,'ano'=>$ano)));//
    }
 ...
 $pdf->Cell(225,5,"-----------------------------------------------------------------------------------------------------------------------------------",0,1);
    ...
    {   $cedula= usuario_actual('cedula');
        ...
        require('/var/www/tcpdf/tcpdf.php');
            ...
            $pdf->Cell(35,5,cambiaf_a_normal($datos_nomina[0]['f_fin']),0,1);
            $pdf->SetFont('courier','B',16);
            $pdf->Cell(0, 17, 'Reporte de Revisión: Codigo Nómina '.$datos_nomina[0]['cod'], 1, 1, 'C');
            ...
            where (n.cod='$cod' and n.tipo_nomina='$tipo_nomina' AND n.cedula='$cedula') and(n.cod_incidencia='7001' or n.cod_incidencia='7002' or n.cod_incidencia='7003' ) order by cod_nomina asc, n.cod_incidencia asc";//lista de nombres apellidos cedula y cod nomina de los integrantes de la nomina
                     $res_cedula=cargar_data($sql,$this);
